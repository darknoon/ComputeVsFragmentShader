...
vertex Fullscreen fullscreenTriangleVertex(ushort id [[ vertex_id ]]) {
    ...
    switch (id) {
        case 0:
            return Fullscreen{.position = float4(minV,minV,0,1), .texCoord = float2(0, 1)};
        case 1:
            return Fullscreen{.position = float4(maxV,minV,0,1), .texCoord = float2(2, 1)};
        default:
        case 2:
            return Fullscreen{.position = float4(minV,maxV,0,1), .texCoord = float2(0, -1)};
    }
}
...
kernel void computeShader012(
    ...
    float2 texCoord = float2(gid) / float2(colorMap.get_width(), colorMap.get_height());

    //    half4 colorSample = texCoord.x + texCoord.y;
    half4 colorSample = colorMap.sample(colorSampler, texCoord.xy);
